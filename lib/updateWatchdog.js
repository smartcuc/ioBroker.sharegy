...
class UpdateWatchdog {
    ...
    async initiateUpdate(targetSpec, timeoutSeconds = 900) {
        const currentVersion = this.adapter.version || "2.2.0";
        ...
        // Execute npm install asynchronously
        const npmCmd = `npm install --save ${target}`;
        this.adapter.log.info(`[OTA Watchdog] Running install: ${npmCmd} in ${installDir}`);

        exec(npmCmd, { cwd: installDir, timeout: 180000 }, (error, stdout, stderr) => {
            if (error) {
                this.adapter.log.error(`[OTA Watchdog] npm install failed: ${error.message}. Aborting update.`);
            ...
            this.adapter.log.info(`[OTA Watchdog] npm install succeeded! Scheduling adapter restart in 1 second...`);
            setTimeout(() => {
    ...
    getStatus() {
        const state = this.readState();
        return {
            current_version: this.adapter.version || "2.2.0",
